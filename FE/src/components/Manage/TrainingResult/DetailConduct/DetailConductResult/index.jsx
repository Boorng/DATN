...
import styles from "./DetailConductResult.module.scss";

const cx = classNames.bind(styles);

function DetailConductResult() {
  const {
    semesterName,
    semesterId,
    academicYear,
  ...
  const getSummary = async () => {
    ...
    if (semesterName[semesterName.length - 1] === "2") {
      average =
        dataResAPI.reduce((sum, current) => sum + current.averageOne, 0) /
        result.length;
    } else {
      average =
        (dataResAPI.reduce((sum, current) => sum + current.averageOne, 0) /
          dataResAPI.length +
          (dataResAPI.reduce((sum, current) => sum + current.averageTwo, 0) /
            dataResAPI.length) *
            2) /
        3;
    }
      ...
      <div ref={pdfRef}>
        <div className={cx("detail-conduct-teacher-header")}>
          <h2 className={cx("detail-conduct-teacher-title")}>
            PHIẾU ĐIỂM HỌC KỲ {semesterName} - NĂM HỌC {academicYear}
          </h2>
          <div className={cx("detail-conduct-teacher-title-wrap")}>
            <Row className="mb-3">
              <span className={cx("detail-conduct-teacher-title-content")}>
        ...
        <div className={cx("detail-conduct-teacher-content")}>
          <table className={cx("detail-conduct-teacher-table")}>
            <thead>
              <tr>
                <th className={cx("detail-conduct-teacher-table-header")}>
                  STT
                </th>
                <th
                  className={cx(
              ...
              {result.map((res, index) => {
                return (
                  <tr key={res.subjectId}>
                    <td className={cx("table-document")}>{index + 1}</td>
                    <td className={cx("table-document")}>{res.subjectName}</td>
                    <td className={cx("table-document")}>{res.averageOne}</td>
                  </tr>
                );
              })}
              <tr>
                <td className={cx("table-document")}></td>
                <td className={cx("table-document", "summary")}>TB các môn</td>
          ...
          <table className={cx("detail-conduct-teacher-table", "conduct")}>
            <thead>
              <tr>
                <th className={cx("detail-conduct-teacher-table-header")}>
                  XẾP LOẠI
            ...
            <tbody>
              <tr>
                <td className={cx("table-document", "summary")}>Học lực</td>
                <td className={cx("table-document")}>{ability}</td>
                <td className={cx("table-document")} rowSpan={3}>
                  {conduct.comment}
                </td>
              </tr>
              <tr>
                <td className={cx("table-document", "summary")}>Hạnh kiểm</td>
                <td className={cx("table-document")}>{conduct.evaluate}</td>
              </tr>
              <tr>
                <td className={cx("table-document", "summary")}>
                  Danh hiệu thi đua
                </td>
                <td className={cx("table-document")}>{title}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default DetailConductResult;
