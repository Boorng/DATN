...
function NavMenu() {
  ...
  return (
    <div className={cx("nav-menu")}>
            ...
            <div className={cx("nav-menu-item")} key={item.id}>
              {item.content ? (
                <div className={cx("nav-menu-item-accordion")}>
                  <div
                    className={cx("nav-menu-item-accordion-header")}
                    onClick={() => handleShowList(item.id)}
                  >
                    <span>{item.title}</span>
                    <FcCollapse
                      className={
                        !showList.includes(item.id) && cx("nav-menu-item-icon")
                      }
                    />
                  </div>
                  {showList.includes(item.id) && (
                    <div className={cx("nav-menu-item-accordion-body")}>
                      {item.content.map((itemChild) => {
                        return (
                          <NavLink
                            key={itemChild.id}
                            className={cx("nav-menu-item-body-child")}
                            to={itemChild.to}
                            style={({ isActive }) =>
      ...
      <a href="/" className={cx("nav-menu-item-href")} onClick={handleLogOut}>
        Đăng xuất
      </a>
    </div>
  );
