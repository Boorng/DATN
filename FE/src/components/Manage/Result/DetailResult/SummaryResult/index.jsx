      ...
      <Table striped hover>
        <thead>
          <tr>
            ...
            <th className={cx("table-head")}>Điểm trung bình học kỳ I</th>
            <th className={cx("table-head")}>Điểm trung bình học kỳ II</th>
        ...
        <tbody>
          {result.map((res) => {
            return (
              <tr key={res.subjectId}>
                ...
                <td className={cx("table-document")}>{res.averageTwo}</td>
          ...
          <tr>
            <td className={cx("table-document", "summary")}>Tổng kết</td>
            <td className={cx("table-document", "summary")}>
              {result.reduce((sum, current) => sum + current.averageOne, 0) /
                result.length}
            </td>
            <td className={cx("table-document", "summary")}>
              {result.reduce((sum, current) => sum + current.averageTwo, 0) /
                result.length}
            </td>
            <td className={cx("table-document", "summary")}>
              {(result.reduce((sum, current) => sum + current.averageOne, 0) /
