...
import StatisticChart from "./DetailResult/StatisticChart";
...
function Result() {
    ...
    useEffect(() => {
        ...
        if (listClass.length > 8) {
            setCurrentItems(listClass.slice(itemOffset, endOffset));
            setPageCount(Math.ceil(listClass.length / itemPerPage));
        } else {
            setItemOffset(0);
            setPageCount(1);
            setCurrentItems(listClass.slice(0, endOffset));
        }
    }, [listClass]);

    const handlePageClick = (event) => {
        const newOffset = (event.selected * itemPerPage) % listClass.length;
        const endOffset = newOffset + itemPerPage;
        setItemOffset(newOffset);
        setCurrentItems(listClass.slice(newOffset, endOffset));
    };
        ...
        <div className={cx("manage-result")}>
            <div className={cx("manage-result-header")}>
                <h2 className={cx("manage-result-title")}>
                    QUẢN LÝ KẾT QUẢ HỌC TẬP KHỐI {gradeName}
                </h2>
            ...
            <div className={cx("manage-result-content")}>
                <div className={cx("list-result")}>
                    <FindResult handleSearch={handleSearch} />
                    <Button
                                    ...
                                    <tr key={cls.id}>
                                        ...
                                        <td className={cx("list-button")}>
                                            <Link
                                                ...
                                                to={`/manage/result/grade/${gradeName}/class/${cls.name}/academicYear/${cls.academicYear}/${cls.id}`}
                                            >
                                                Xem chi tiết
                                            </Link>
                                        </td>
